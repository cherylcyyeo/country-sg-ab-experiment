WITH acc AS (
  ...
  WHERE created_date_utc >= '2022-09-30'
...
vouchers AS (
  ...
  WHERE global_entity_id = 'FP_SG'
    AND created_date_utc BETWEEN '2022-09-30' AND '2022-12-01'
),
...
discounts AS (
  ...
  WHERE global_entity_id = 'FP_SG'
    AND created_date_utc >= '2022-09-30'
),
...
vendor_classification AS (
  SELECT DISTINCT
    vendor_code, 
    name, 
    IFNULL(chain_code, vendor_code) chain_code, 
    IFNULL(chain_name, name) chain_name, 
    tags.title,
    CASE 
...
SELECT
  ...
  order_hour AS hour_,
  CASE 
    WHEN EXTRACT(HOUR FROM order_report_time) IN (7, 8, 9, 10) THEN "Breakfast"
...
INNER JOIN vendors ON orders.ga_vendor_code = vendors.vendor_code
LEFT JOIN `fulfillment-dwh-production.pandata_report.country_SG_rs_gross_profit_per_order` gpo ON orders.platform_order_code = gpo.order_code
LEFT JOIN pd_orders ON orders.platform_order_code = pd_orders.code
